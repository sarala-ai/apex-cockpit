...
// skills/ and .agents/skills/ live at the monorepo root, not under server/.
// CI's canonical invocation (scripts/run-vitest-stable.mjs) runs vitest with
// cwd = repo root, so process.cwd()-relative paths resolve correctly there —
// but running vitest directly from server/ (`cd server && npx vitest run`)
// leaves process.cwd() at server/, where these paths don't exist. Same fixup
// already used in plugin-database.test.ts for the same reason.
const repoRoot =
...
describe("paperclip skill utils", () => {
  ...
  it("documents artifact uploads in the installed Paperclip skill", async () => {
    ...
    const referenceBody = await fs.readFile(
      path.resolve(repoRoot, "skills/paperclip/references/artifacts.md"),
      "utf8",
    );
    ...
    await expect(
      fs.access(path.resolve(repoRoot, "scripts/paperclip-upload-artifact.sh")),
    ).rejects.toThrow();
  });
  ...
  it("keeps the create-issue-interaction-ui guide as a maintainer-only skill", async () => {
    ...
    await expect(
      fs.access(path.resolve(repoRoot, "skills/create-issue-interaction-ui/SKILL.md")),
    ).rejects.toThrow();
  });
