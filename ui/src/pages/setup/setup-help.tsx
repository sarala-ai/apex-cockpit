...
export const STEP_HELP: Record<StepKey, StepHelp> = {
  claudeSession: {
    ...
    whatYoullDo: [
      ...
      "In a plain browser instead: run `apex claude connect --cockpit-url <this cockpit> --org-id <org>` in your own terminal (or `--company-id <company>` if you're not org-scoped yet) and follow its page (it adds one cockpit CLI-access approval).",
  ...
  auth: {
    ...
    whatYoullDo: [
      "Open the APEX desktop app: it reports your gcloud/GitHub/ADC state and offers Sign in to Google Cloud — that sign-in also sets Application Default Credentials (ADC).",
      "Terminal-first? Run `apex connect gcloud` to sign in and set ADC in one step, then `apex doctor --report` to send the report from the command line.",
      "GitHub connect isn't in-app yet — run `apex connect github`.",
    ],
    note: "ADC isn't part of identity and never blocks this step — it's set by the same gcloud sign-in above, and it's what the cloud-provisioning steps further down will need once they execute. Discovery and the Artifact Registry pull read your local gcloud / gh credentials.",
  },
  org: {
    purpose:
  ...
  oauthClient: {
    ...
    whatYoullDo: [
      "Once configured, this step just confirms it — the row collapses to a one-line summary with an Edit action.",
      "Not configured yet: this is a one-time, manual setup on your Google Cloud project (no APEX workflow automates it yet) — create a Web-application OAuth client, then store its client id and secret in Secrets.",
    ],
    note: "Whoever administers the Google Cloud project does this once; it isn't something this cockpit runs for you.",
    links: [
  ...
  mcpServers: {
    purpose:
      "Register the MCP servers whose tools your agents should see (e.g. Google Workspace). The gateway federates their tools into one catalog. Registered alone isn't enough — this step is done only once every registered upstream also answers reachable.",
    whatYoullDo: [
      "cockpit-mcp is built in — the gateway derives it automatically, no action needed here.",
      "For any other upstream: Gateway page → Add upstream, with its endpoint + transport.",
      "The gateway auto-connects, lists its tools, and federates them — check reachability on the Gateway page if a row shows unreachable.",
    ],
  },
  connect: {
    purpose:
  ...
  models: {
    purpose:
      "Answer 'how do model calls get paid for and routed here' — on a hosted cockpit that's the connected Claude subscription (next step) or an API key; on a local cockpit it's the subscription bridge.",
    whatYoullDo: [
      "On a hosted cockpit: model calls use your connected Claude session — see the 'Connect Claude subscription' step below. An API key or OpenRouter (Advanced fold) also works.",
      ...
      "Advanced (Claude API key): paste the key here, or set it directly on the Secrets page — either way it's forwarded ONLY to the gateway's encrypted store, the cockpit never persists it. Enables per-token cost attribution (Costs page).",
      "Advanced (OpenRouter): paste the key here, or set it on the Secrets page, to register a BYO-plane provider. Existing `apex-*` aliases stay on Claude subscription; OpenRouter is available as an additional provider.",
    ],
